...
| `APP_JWT_EXPIRES_IN`                          | durée de vie du JWT applicatif (`15m` par défaut) — les sessions longues passent par le refresh token (`POST /auth/refresh`, révocable, table `refresh_token`), pas par ce TTL                                                                                                                 |
